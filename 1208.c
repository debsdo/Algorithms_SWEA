...
int minIdxChecker(int board[][100]) {
	int min = minChecker(board);
	for (int i = 0; i < 100; i++) {
		int cnt = 0;
		for (int j = 0; j < 100; j++) {
			if (board[i][j] == 1) {
...
int maxIdxChecker(int board[][100]) {
	int max = maxChecker(board);
	for (int i = 0; i < 100; i++) {
		int cnt = 0;
		for (int j = 0; j < 100; j++) {
			if (board[i][j] == 1) {
...
int main() {
	for (int t = 0; t < 10; t++) {
		...
		for (int i = 0; i < dump; i++) {
			int min = minChecker(board);
			int max = maxChecker(board);
			int maxIdx = maxIdxChecker(board);
			int minIdx = minIdxChecker(board);
			board[maxIdx][max-1] = 0;
			board[minIdx][min] = 1;
		}

		int gap = maxChecker(board) - minChecker(board);
		printf("#%d %d\n", t + 1, gap);
	}
