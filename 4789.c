...
// 그래서 이 시점에서 2명을 더 고용하면 총 5명이 박수를 치게 된다.
// 그 다음으로 박수를 칠 사람은 5명 이상일 때에만 박수를 치는데 현재 5명이 박수를 치고 있으므로 마찬가지로 박수를 칠 것이다.
...
int main() {
	...
	for (int t = 0; t < tc; t++) {
		...
		if (people[0] - 48 == 0) {
			...
			clapping += (people[0] - 48);
		}

		for (int i = 1; i < strlen(people); i++) {
			int num = people[i] - 48;
			...
			if (num != 0) {
				if (clapping >= i) {
					...
					int albaAdd = i - clapping;
					alba += albaAdd;
					clapping += (albaAdd + num);
				}
			}
		}
