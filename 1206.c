...
//연두색으로 색칠된 여섯 세대에서는 좌우로 2칸 이상의 공백이 존재하므로 조망권이 확보된다.
//따라서 답은 6이 된다.
...
//[입력]
//입력 파일의 첫 번째 줄에는 테스트케이스의 길이가 주어진다.
//그 바로 다음 줄에 테스트 케이스가 주어진다.
//총 10개의 테스트케이스가 주어진다.

//[출력]
//#부호와 함께 테스트 케이스의 번호를 출력하고,
//공백 문자 후 테스트 케이스의 조망권이 확보된 세대의 수를 출력한다.
...
int main() {
	for (int t = 0; t < 10; t++) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < 255; j++) {
				if (bldgs[i][j] == 1) {
					if (bldgs[i - 1][j] == 0 && bldgs[i - 2][j] == 0 && bldgs[i + 1][j] == 0 && bldgs[i + 2][j] == 0) {
						cnt += 1;
					}
				}
				else {
